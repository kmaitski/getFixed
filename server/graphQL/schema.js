...
const resolvers = {
  Query: {
    ...
    problemMessages: (obj, args, context) => {
      const messages = tempMessages[args.id]
        ? { id: args.id, messages: tempMessages[args.id] }
        : { id: args.id, messages: [] };
      return messages;
    },
    nearByListings: (obj, args) => {
      ...
      const filterByDistance = (
        ...
        const distanceBetweenLat = toRad(
          listingCoordinates[0] - userCoordinates[0]
        );
        const distanceBetweenLong = toRad(
          listingCoordinates[1] - userCoordinates[1]
        );
        const haversineFormula =
          Math.sin(distanceBetweenLat / 2) * Math.sin(distanceBetweenLat / 2) +
          Math.cos(userRadianLat) *
            Math.cos(userRadianLat) *
            Math.cos(listingRadianLat) *
            Math.sin(distanceBetweenLong / 2) *
            Math.sin(distanceBetweenLong / 2);
        const nextStep =
          2 *
          Math.atan2(
              ...
              if (listings[i].dataValues.latitude) {
                if (
                  filterByDistance(
                    ...
                    [
                      listings[i].dataValues.latitude,
                      listings[i].dataValues.longitude
                    ],
                    args.radius * 1609.34
                  )
                ) {
      ...
      if (args.latitude !== null) {
        // console.log(1);
        return db.listings.findAll().then(listings => {
          const filteredListings = [];
          for (let i = 0; i < listings.length; i += 1) {
            if (filteredListings.length === 25) {
              return filteredListings;
